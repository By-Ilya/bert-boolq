...
flags.DEFINE_bool("do_eval_test", False,
...
def _create_examples(filename, type_of_set):
    examples = []
    with tf.gfile.Open(filename) as f:
        for i, line in enumerate(f):
            data = json.loads(line)
            guid = "%s-%s" % (type_of_set, i)
    ...
    if FLAGS.n_examples and type_of_set == "train":
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    if ex_index < 5:
        ...
        tf.logging.info("input mask: %s" % " ".join(
...
def file_based_convert_examples_to_features(examples, label_list, max_seq_length,
...
def create_model(bert_config, is_training, input_ids, input_mask,
                 segment_ids, labels, num_labels, use_one_hot_encodings):
    model = modeling.BertModel(
        ...
        use_one_hot_embeddings=use_one_hot_encodings)
